...
namespace CCAPPUI.Controllers
{
	...
	{
		[HttpPost]
		public string save([FromBody] BrandBLL obj)
		{
			return BrandDAL.save(obj);
		}
		[HttpPost]
		public string delete([FromBody] BrandBLL obj)
		{
			return BrandDAL.delete(obj);
		}

		[HttpGet]
		public List<BrandBLL> List()
		{
			return BrandDAL.List();
		}

		[HttpPost]
		public string update([FromBody] BrandBLL obj)
		{
			return BrandDAL.update(obj);
		}
		...
		[HttpPost]
		public async Task<HttpResponseMessage> UploadBrandExel()
		{
			string location = "";
			string fileName = "";
			...
			var Obj = JsonConvert.DeserializeObject<BrandBLL>(model);

			DataTable tbl = new DataTable();
			DataColumn col = new DataColumn();
			col.ColumnName = "Brand_ID";
			...
			try
			{
				...
				for (int i = 0; i < result.FileData.Count; i++)
				{
					...
					if (i == 0)
					{
						...
						var sheet = excel.Workbook.Worksheets["Brand"];
						if (sheet == null)
						{
							return Request.CreateResponse(HttpStatusCode.OK, "2");
						}
						...
						{
							if (tbl.Columns[1].ColumnName != "Brand Name")
							{
